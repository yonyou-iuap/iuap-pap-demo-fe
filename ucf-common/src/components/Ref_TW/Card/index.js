    ...
    render(){
        ...
        return (
            <Panel
                ...
                header={<div><span className="card-title">{title} </span><span onClick={this.showCode} className="card-check-code">查看源码_tw</span></div>}
                {...opt}
            >
            {
                this.props.children
            }
                <Drawer title={'示例源码_tw'} show={this.state.showDrawer} placement={'right'} onClose={this.fCloseDrawer}>
                    <pre>
